...
const log = require("../logging").get("IrcHandler");

const JOIN_QUEUE_CONCURRENCY = 100;
const PART_QUEUE_CONCURRENCY = 100;
...
function IrcHandler(ircBridge) {
    ...
    // QueuePools for join and part, two endpoints that get absolutely hammered on mass 
    // connects/disconnects.
    this._reqQueue = {
        "join": new QueuePool(JOIN_QUEUE_CONCURRENCY, (obj) => {
            if (this._reqQueue["join"].waitingItems > 0) {
                log.debug(`joins in queue ${this._reqQueue["join"].waitingItems}`);
            }
            return this._onJoin(
                obj.req,
                obj.server,
        ...
        "part": new QueuePool(PART_QUEUE_CONCURRENCY, (obj) => {
            if (this._reqQueue["part"].waitingItems > 0) {
                log.debug(`parts in queue ${this._reqQueue["part"].waitingItems}`);
            }
            return this._onPart(
                obj.req,
                obj.server,
...
IrcHandler.prototype._ensureMatrixUserJoined = Promise.coroutine(function*(roomId,
    ...
    if (!priv) {
        ...
        _log.info("Querying PM room state (%s) between %s and %s",
    ...
    if (priv.membership !== "join" && priv.membership !== "invite") { // fix it!
        _log.info("Inviting %s to the existing PM room with %s (current membership=%s)",
...
IrcHandler.prototype.onJoin = Promise.coroutine(function*(req, server, joiningUser, chan, kind) {
    ...
    return this._reqQueue["join"].enqueue(
        req.req.getId(),
        {
            req:req,
            server:server,
 ...
 * @return {Promise} which is resolved/rejected when the request finishes.
 */
IrcHandler.prototype._onJoin = Promise.coroutine(function*(req, server, joiningUser, chan, kind) {
...
IrcHandler.prototype.onPart = Promise.coroutine(function*(req, server, leavingUser, chan, kind) {
    ...
    return this._reqQueue["part"].enqueue(
        req.req.getId(),
        {
            req:req,
            server:server,
...
IrcHandler.prototype._onPart = Promise.coroutine(function*(req, server, leavingUser, chan, kind) {
    // parts are always incremental (only NAMES are initial)
    if (!server.shouldSyncMembershipToMatrix("incremental", chan)) {
        req.debug.debug("Server doesn't mirror parts.");
        return;
    }
