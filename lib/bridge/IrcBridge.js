...
const RemoteClientPool = require("../irc/remote/RemoteClientPool");
var IrcEventBroker = require("../irc/IrcEventBroker");
const BridgedClient = require("../irc/BridgedClient");
const RemoteBridgedClient = require("../irc/remote/RemoteBridgedClient");
...
function IrcBridge(config, registration) {
    ...
    const connTrackConfig = this.config.ircService.connectionTracker;
    if (connTrackConfig && connTrackConfig.enabled) {
        this._clientPool = new RemoteClientPool(this, connTrackConfig);
    } else {
...
IrcBridge.prototype.createBridgedClient = function(ircClientConfig, matrixUser, isBot) {
    ...
    if (this._clientPool.isRemote) {
        return new RemoteBridgedClient(
            this._clientPool, server, ircClientConfig, matrixUser, isBot,
...
IrcBridge.prototype.run = Promise.coroutine(function*(port) {
    ...
    // Connect the ClientPool *if* we using a remote one.
    if (this._clientPool.isRemote) {
        try {
            yield this._clientPool.connect(this.ircServers);
        }
        catch {
            throw new Error("Error connecting to the remote connection tracker.");
        }
    }
