...
const ChannelCache = require("irc").ChannelCache;
...
function ClientPool(ircBridge) {
    ...
    this._channelCaches = {
        // server_domain = ChannelCache;
    }
    // This counter should forever increase.
    this.client_index = 0;
}
...
ClientPool.prototype.createIrcClient = function(ircClientConfig, matrixUser, isBot) {
    this.client_index++;
    let channelCache = this._channelCaches[ircClientConfig.getDomain()];
    if (channelCache === undefined) {
        channelCache = this._channelCaches[ircClientConfig.getDomain()] = new ChannelCache();
    }

    var bridgedClient = this._ircBridge.createBridgedClient(
        ircClientConfig, matrixUser, isBot, channelCache, this.client_index
    );
...
ClientPool.prototype._removeBridgedClient = function(bridgedClient) {
    ...
    this._virtualClientCounts[server.domain] = this._virtualClientCounts[server.domain] - 1;
    // Remove lingering channel cache entries.
    this._channelCaches[server.domain].getChannelList(bridgedClient.clientId).forEach((channame) =>  {
        this._channelCaches[server.domain].markClientLeft(bridgedClient.clientId, channame);
    });
