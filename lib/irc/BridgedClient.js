...
function BridgedClient(server, ircClientConfig, matrixUser, isBot, eventBroker, identGenerator,
    ...
    this.nick = BridgedClient.getValidNick(
...
BridgedClient.prototype.changeNick = function(newNick, throwOnInvalid) {
    ...
    try {
        validNick = BridgedClient.getValidNick(newNick, throwOnInvalid, this.unsafeClient);
 ...
 * back to a user.
 */
BridgedClient.getValidNick = function(nick, throwOnInvalid, unsafeClient) {
    ...
    if (unsafeClient) {
        ...
        if (unsafeClient.supported &&
                typeof unsafeClient.supported.nicklength == "number") {
            maxNickLen = unsafeClient.supported.nicklength;
        }
