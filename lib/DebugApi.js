...
DebugApi.prototype.run = function() {
    ...
    http.createServer((req, response) => {
        try {
            ...
            else if (req.method === "POST" && path == "/memberSync") {
                this.resyncMembers(req, response);
                return;
            }
...
DebugApi.prototype.resyncMembers = Promise.coroutine(function*(req, response) {
    const body = yield this._wrapJsonReq(req, response);
    const syncers = this.ircBridge.memberListSyncers;
    if (typeof(body.domain) !== "string") {
        this._wrapJsonResponse({"err": "'domain' was not given or not a string"}, false, response);
        return;
    }
    else if (syncers[body.domain] === undefined) {
        const options = Object.keys(syncers).join(", ");
        this._wrapJsonResponse({"err": `'domain' should be one of ${options}.`}, false, response);
        return;
    }
    try {
        yield syncers[body.domain].sync();
        log.info(`Resyncing members for ${body.domain}!`);
        this._wrapJsonResponse({"msg":"Success."}, true, response);
    }
    catch (e) {
        this._wrapJsonResponse({"err":"Failed", "ex": e}, false, response);
    }
});
